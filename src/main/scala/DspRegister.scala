...
trait DspRegisterFromDsptools[D, U, EO, EI, B <: Data] extends DspBlock[D, U, EO, EI, B] {
  /**
...
trait DspRegisterFromDsptoolsImp[D, U, EO, EI, B <: Data] extends LazyModuleImp with HasRegMap {
  def outer: DspRegisterFromDsptools[D, U, EO, EI, B] = wrapper.asInstanceOf[DspRegisterFromDsptools[D, U, EO, EI, B]]
  ...
  require(streamEdgeIn.bundle.hasData, "DspRegisterFromDsptools registers data, must have data field!")
  require(!streamEdgeIn.bundle.hasKeep, "DspRegisterFromDsptools does not support TKEEP")
  require(!streamEdgeIn.bundle.hasStrb, "DspRegisterFromDsptools does not support TSTRB")
  require(streamEdgeIn.bundle.i == 0 && streamEdgeIn.bundle.d == 0, "DspRegisterFromDsptools does not support TID or TDEST")
...
class TLDspRegisterFromDsptools(val len: Int, val mapMem: Boolean = true, val baseAddr: BigInt = 0, devname: String = "vreg", concurrency: Int = 1)(implicit p: Parameters)
  extends TLRegisterRouter(baseAddr, devname, Seq("ucb-bar,vreg"), beatBytes = 8, concurrency = concurrency)(
    ...
    new TLRegModule(len, _, _)
      with DspRegisterFromDsptoolsImp[TLClientPortParameters, TLManagerPortParameters, TLEdgeOut, TLEdgeIn, TLBundle]
  ) with DspRegisterFromDsptools[TLClientPortParameters, TLManagerPortParameters, TLEdgeOut, TLEdgeIn, TLBundle] with TLDspBlock {
  override val mem = Some(node)
}

class AXI4DspRegisterFromDsptools(val len: Int, val mapMem: Boolean = true, val baseAddr: BigInt = 0, concurrency: Int = 4)(implicit p: Parameters)
  extends AXI4RegisterRouter(baseAddr, beatBytes = 8, concurrency = concurrency)(
    new AXI4RegBundle(len, _))(
    new AXI4RegModule(len, _, _)
      with DspRegisterFromDsptoolsImp[AXI4MasterPortParameters, AXI4SlavePortParameters, AXI4EdgeParameters, AXI4EdgeParameters, AXI4Bundle]
  ) with DspRegisterFromDsptools[AXI4MasterPortParameters, AXI4SlavePortParameters, AXI4EdgeParameters, AXI4EdgeParameters, AXI4Bundle]
    with AXI4DspBlock {
  val mem = Some(node)
}
