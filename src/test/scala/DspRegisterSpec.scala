...
class DspRegisterFromDsptoolsTestModule(
  ...
  val lazyMod = LazyModule(new LazyModule() {
    ...
    val reg    = LazyModule(new AXI4DspRegisterFromDsptools(len))
...
class DspRegisterFromDsptoolsTestModuleTester(c: DspRegisterFromDsptoolsTestModule,
...
class DspRegisterFromDsptoolsSpec extends FlatSpec with Matchers {
  behavior of "AXI4DspRegisterFromDsptools"
  ...
  it should "be able to read and write" in {
    ...
    chisel3.iotesters.Driver(() => new DspRegisterFromDsptoolsTestModule(inP, outP, 64, transactions), backendType = "verilator") {
      c => new DspRegisterFromDsptoolsTestModuleTester(c) {
        axiWriteWord(0, 64)
        axiWriteWord(0x10, 15)
        //axiWriteWord(0x8, 0xFF00)
        axiWriteWord(0x8, 0xFFFF) // load and store at the same time
        step(64)
