...
// This test example is similar like DspRegisterSpec defined in
// https://github.com/ucb-bar/dsptools/blob/211166e635861fb1937828aee38c166baf0840b5/rocket/src/test/scala/dspblocks/DspRegisterSpec.scala

class DspFIFOTestModule(
  ...
  val lazyMod = LazyModule(new LazyModule() {
    ...
    val reg    = LazyModule(new AXI4DspFIFO(len))
...
class DspFIFOTestModuleTester(c: DspFIFOTestModule,
...
class DspFIFOSpec extends FlatSpec with Matchers {
  behavior of "AXI4DspFIFO"

  it should "be able to read and write" in {
    val inP  = AXI4StreamBundleParameters(n = 4)
    val outP = AXI4StreamSlaveParameters()
    val transactions = AXI4StreamTransaction.defaultSeq(64).zipWithIndex.map({case (t, i) => t.copy(data = i) })

    chisel3.iotesters.Driver(() => new DspFIFOTestModule(inP, outP, 64, transactions), backendType = "verilator") {
      c => new DspFIFOTestModuleTester(c) {
        axiWriteWord(0, 64)
        axiWriteWord(0x8, 15)
        axiWriteWord(0x4, 0xFF00)
        step(64)
        axiWriteWord(0x4, 0x00FF)
        stepToCompletion()

        for (i <- 0 until 64) {
          //require(axiReadWord(12 + i * 4) == BigInt(i), s"Addr $i is wrong")
        }
      }
    } should be (true)
  }
